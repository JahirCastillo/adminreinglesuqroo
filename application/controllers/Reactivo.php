...
class Reactivo extends CI_Controller
{
    ...
    public function lista()
    {
        ...
        $rResult = $this->generico_model->datosDataTable($aColumns, $sTable, $sWhere, $sOrder, $sLimit, $rol, $this->user_id, "rea_usuarioalta");
        ...
        for ($x = 0; $x < $rResult->num_rows(); $x++) {
            ...
            $html_st = "<button class='btn btn-warning' onclick='open_in_new(\"reactivo/update/" . $aRow[$sIndexColumn] . "\")'><i class='fa fa-pencil'></i></button><button class='btn btn-danger' onclick='deleteRea(\"" . $aRow[$sIndexColumn] . "\")'><i class='fa fa-trash'></i></button>";
    ...
    {
        $id = $id * 1;
        $datos_vista['plan_id'] = $this->input->post('plan_id');
        $datos_vista['plan_nombre'] = $this->input->post('plan_nombre');
        ...
        $datos_vista['lista'] = $this->plan_model->baseTree($rol, $user_id)->result_array();
        $datos_vista['plan'] = $this->getActualPlan();
        $datos_vista['siguiente'] = $this->reactivo_model->getSiguiente($id);
        $datos_vista['anterior'] = $this->reactivo_model->getAnterior($id);
        $datos_vista['habilidades'] = $this->reactivo_model->getHabilidades($id, 0);
        ...
        $datos_vista['materias'] = $this->reactivo_model->getMaterias();
        $datos_vista['competencias'] = $this->reactivo_model->getCompetenciasMateria();
        $datos_vista['bloques'] = $this->reactivo_model->getBloquesCompetencia();

        $datos_vista['reactivo_en_examen'] = $this->reactivo_model->getExamenReactivo($id);
        if (array_key_exists($this->clave_modulo, $permisos)) {
            $datos_vista['permisos_modulo'] = $permisos[$this->clave_modulo];
        }
        $actionTxt = 'Agregar';
        ...
        $datos_plantilla['content'] = $this->load->view('reactivo/reactivo_view', $datos_vista, true);
        $this->load->view('template', $datos_plantilla);
    }
    ...
    function guardarReactivo()
    {
        ...
        $datos = array( //arreglo datos del reactivo
            ...
            'rea_habilidad' => $this->input->post('idHab'),
            'rea_txthabilidad' => $this->input->post('txtHab'),
            'rea_idpadrehab' => $this->input->post('idPadreHab'),
            'rea_txtpadrehab' => $this->input->post('txtPadreHab'),
            'rea_id_materia' => $this->input->post('idMateriaRea'),
            'rea_texto_materia' => $this->input->post('textoMateriaRea'),
            'rea_id_competencia' => $this->input->post('idCompetenciaMateria'),
            'rea_texto_competencia' => $this->input->post('textoCompetenciaMateria'),
            'rea_id_bloque' => $this->input->post('idBloque'),
            'rea_texto_bloque' => $this->input->post('textoBloque'),
            'rea_autor' => $this->input->post('aclave'),
            'rea_puntos' => $this->input->post('puntosReactivo')
        );
        $datos_notas["nota_descripcion"] = str_replace(array("\r\n", "\r", "\n", "\\n"), "", $this->input->post('textonotas'));
        $datos_notas["nota_descripcion"] = str_replace("'", '"', $datos_notas["nota_descripcion"]);
        $existe = 0;
        if (is_numeric($clave)) {
            $existe = (($clave * 1) != 0) ? $this->reactivo_model->existeReactivo($clave) : 0;
        }
        if ($existe > 0) { // modifica reactivo
            $datos['rea_fechamodif'] = $fecha;
            $datos['rea_usuariomodif'] = $user_id;
            $sepudo = $this->reactivo_model->modificaReactivo($clave, $datos, $datos_notas);
            if ($sepudo !== FALSE) {
                $json_out['resp'] = 'ok';
                $json_out['act'] = 'upd';
            ...
            $idinsert = $this->reactivo_model->insertarReactivo($datos, $datos_notas);
            if ($idinsert != FALSE) {
                $json_out['resp'] = 'ok';
                $json_out['act'] = 'add';
    ...
    function llenarReactivo()
    {
        ...
        $datos['textoNotasRea'] = $this->reactivo_model->getNotasReactivo($idrea)->row_array();
        if (empty($datos['textoNotasRea'])) {
            $datos['textoNotasRea']['notas'] = "";
        }

        $idRol = $this->session->userdata('id_rol' . $this->clv_sess);
        $datos['permisoVerReactivo'] = $this->reactivo_model->permisoVerReactivo($idRol, $datos['pid']);
    ...
    protected function get_full_url()
    {
        ...
        return ($https ? 'https://' : 'http://') . (!empty($_SERVER['REMOTE_USER']) ? $_SERVER['REMOTE_USER'] . '@' : '') . (isset($_SERVER['HTTP_HOST']) ? $_SERVER['HTTP_HOST'] : ($_SERVER['SERVER_NAME'] . ($https && $_SERVER['SERVER_PORT'] === 443 ||
    ...
    function copy()
    {
        $idNodoACopiar = $this->input->post('nodoACopiar') * 1;
        $idPlan = $this->input->post('nodoDestino') * 1;
        $clv_sess = $this->config->item('clv_sess');
        $user_id = $this->session->userdata('user_id' . $clv_sess);
        $insert = $this->reactivo_model->copyReactivo($idNodoACopiar, $idPlan, $user_id);
        if ($insert != FALSE) {
            $array_out['res'] = 'ok';
        } else {
            $array_out['res'] = 'no';
            $array_out['msg'] = 'Error al copiar el reactivo. Intenta de nuevo.';
        }
        echo json_encode($array_out);
    }

    function desplegarPadres()
    {
        $idPlan = $this->input->post('idPlan') * 1;
        $padding = $this->input->post('padding') * 1;
        $this->load->model('plan_model');
        $lista = '';
        $datosPadres = $this->plan_model->obtenerPadres($idPlan);
        if ($datosPadres != FALSE) {
            $padding = $padding - 5;
            $lista = " <li class='elementoPadre' value='" .
                $datosPadres->id . "' style='padding-left:" . $padding . "%;'>" .
                $datosPadres->nombrePlan . "</li>";
            echo $lista;
        }
        //echo $lista->padre;
        //$list = '<ul class="nav">';
    ...
    {
        $idMateria = $this->db->escape_str($this->input->post('idMateria'));
        $competencias = $this->reactivo_model->getCompetenciasMateria($idMateria);
        $opciones = "";
        foreach ($competencias as $competencia) {
            $opciones .= "<option title='" . $competencia['comp_mat_descripcion'] . "' value='" . $competencia['comp_mat_id'] . "'>" . $competencia['comp_mat_nombre'] . "</option>";
        }
        echo $opciones;
    }
    function getBloques()
    {
        $idCompetencia = $this->db->escape_str($this->input->post('idCompetencia'));
        $bloques = $this->reactivo_model->getBloquesCompetencia($idCompetencia);
        $opciones = "";
        foreach ($bloques as $bloque) {
            $opciones .= "<option title='" . $bloque['bloque_comp_proposito'] . "' value='" . $bloque['bloque_comp_id'] . "'>" . $bloque['bloque_comp_nombre'] . "</option>";
        }
        echo $opciones;
    }
    function getConocimiento()
    {
        $idBloque = $this->db->escape_str($this->input->post('idBloque'));
        $conocimiento = $this->reactivo_model->getConocimientoBloque($idBloque);
        $textoConocimiento = "<label>Conocimiento</label><br>" . $conocimiento[0]['con_hab_conocimiento'] . "<br><br><label>Habilidad</label><br>" . $conocimiento[0]['con_hab_habilidad'];

        echo $textoConocimiento;
    }
}
